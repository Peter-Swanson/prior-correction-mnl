...
When modeling low event rate data with logistic regression, it is common to use stratified sampling on the target variable, $Y$. This could mean taking all of the cases where $Y=1$, and a random sample of the cases where $Y=0$ to build a modeling dataset. This new dataset would have the wrong marginal distribution of the target, and unadjusted logistic regression models would overpredict $P(Y=1)$. Fortunately, this is easily remedied by {\bf prior correction} where we essentially add a correction factor to the linear component of the logistic regression model. This correction factor only needs the marginal distribution of the target variable as it was run through the model (posteriors) and the ``true" population (prior) distribution of the target. In practice, the priors are often estimated from the data before sampling.

This process allows us to ``correct", ``adjust", ``calibrate", or ``tune" any logistic regression model to any prior distribution of $Y$. For example, in credit risk, we can use it to calibrate a logistic regression model to any pre-specified default rate.
...
Additionally, a method is proposed for calculating prior probabilities from an incomplete vector of pre-specified probabilities by making adjustments to the observed marginals in the model build data. For example, we may have models predicting five levels: default, payoff, upgrade, downgrade, no-change, but we may only want to calibrate a model's default rate. The method proposed below allows an analyst to specify prior probabilities for any of the levels, and automatically calculates the others in a consistent way that keeps the sum of the probabilities equal to one. 
